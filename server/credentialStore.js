...
export async function setPassword(accountId, password, options = {}) {
  ...
  const record = {
    ...
    passwordHistory: [current.passwordHash, ...(current.passwordHistory || [])].filter(Boolean).slice(0, HISTORY_LIMIT),
    updatedAt: nowIso(), passwordChangedAt: nowIso(), passwordExpiresAt: options.passwordExpiresAt || null,
    forcePasswordReset: options.forcePasswordReset === true, failedAttempts: 0, lockedUntil: null,
  }
...
export async function removeCredential(accountId) {
  ...
  const next = { ...credentials }; delete next[id]; await writeJson(CREDENTIAL_FILE, next); return true
}
export async function migratePlaintextCredentials() {
  const accounts = await readJson(paths.clients(), []); const credentials = await readJson(CREDENTIAL_FILE, {})
  let accountChanged = false; let credentialChanged = false; const migrated = []
