...
const DEFAULT_POLICY = { id: 'global', name: 'Global API protection', route: '*', methods: ['*'], subjectTypes: ['ip'], windowMs: 60_000, maximum: 300, blockMs: 300_000, enabled: true, priority: 0 }

export class AbuseProtectionError extends Error {
  constructor(message, status = 400, details = null) { super(message); this.name = 'AbuseProtectionError'; this.status = status; this.details = details }
}
...
async function readRegistry() {
  ...
  registry.policies ||= [DEFAULT_POLICY]; registry.counters ||= {}; registry.blocks ||= []; registry.overrides ||= []; registry.history ||= []; registry.trustedProxies ||= []; registry.version ||= 1
  return registry
}
async function mutate(operation) {
  ...
  const current = previous.catch(() => {}).then(async () => {
    ...
    for (const [key, timestamps] of Object.entries(registry.counters)) {
      const active = Array.isArray(timestamps) ? timestamps.filter(value => Number(value) >= stale) : []
      if (active.length) registry.counters[key] = active; else delete registry.counters[key]
    }
...
function normalisePolicy(input = {}, existing = null) {
  ...
  return { id, name: String(input.name ?? existing?.name ?? id).trim().slice(0, 200), route, methods, subjectTypes, windowMs: Math.min(86_400_000, Math.max(1000, Number(input.windowMs ?? existing?.windowMs ?? 60_000))), maximum: Math.min(1_000_000, Math.max(1, Number(input.maximum ?? existing?.maximum ?? 100))), blockMs: Math.min(30 * 86_400_000, Math.max(1000, Number(input.blockMs ?? existing?.blockMs ?? 300_000))), enabled: input.enabled === undefined ? existing?.enabled !== false : input.enabled === true, priority: Math.min(10_000, Math.max(-10_000, Number(input.priority ?? existing?.priority ?? 0))) }
}
function routeMatches(pattern, route) { return pattern === '*' || (pattern.endsWith('*') ? route.startsWith(pattern.slice(0, -1)) : route === pattern) }
function policyMatches(policy, route, method) { return policy.enabled && routeMatches(policy.route, route) && (policy.methods.includes('*') || policy.methods.includes(method)) }
function stripMappedAddress(value) { return String(value || '').replace(/^::ffff:/, '') }
function trustedProxy(address, configured) { const candidate = stripMappedAddress(address); return configured.some(item => stripMappedAddress(item) === candidate) }
...
function subjects(context) {
  return [context.ip && { type: 'ip', id: context.ip }, context.sessionId && { type: 'session', id: context.sessionId }, context.accountId && { type: 'account', id: context.accountId }, context.apiKeyId && { type: 'api-key', id: context.apiKeyId }].filter(Boolean)
}
function overrideMatches(item, subject, id) { return item.subjectType === subject.type && item.subjectId === subject.id && (!item.policyId || item.policyId === id) && (!item.expiresAt || new Date(item.expiresAt).getTime() > Date.now()) }
...
export async function upsertAbusePolicy(input = {}, actor = null) {
  return mutate(registry => {
    const id = policyId(input.id); const existing = registry.policies.find(item => item.id === id)
...
export async function updateTrustedProxies(values = [], actor = null) {
  ...
  return mutate(registry => { registry.trustedProxies = proxies; registry.history.unshift({ id: crypto.randomUUID(), action: 'trusted-proxies.updated', count: proxies.length, actor, createdAt: nowIso() }); return proxies })
}
export async function setAbuseOverride(input = {}, actor = null) {
  const subjectType = text(input.subjectType, 'Subject type', 30); if (!SUBJECT_TYPES.has(subjectType)) throw new AbuseProtectionError('Override subject type is invalid', 422)
  const subjectId = text(input.subjectId, 'Subject ID', 500); const mode = text(input.mode, 'Override mode', 20)
  ...
  return mutate(registry => { const record = { id: crypto.randomUUID(), subjectType, subjectId, policyId: input.policyId ? policyId(input.policyId) : null, mode, reason: String(input.reason || '').trim().slice(0, 500) || null, expiresAt: expiresAt?.toISOString() || null, createdAt: nowIso(), createdBy: actor }; registry.overrides.unshift(record); registry.history.unshift({ id: crypto.randomUUID(), action: `abuse-override.${mode}`, overrideId: record.id, subjectType, policyId: record.policyId, actor, createdAt: nowIso() }); return record })
}
  ...
  return mutate(registry => {
    ...
    for (const policy of policies) {
      for (const subject of requestSubjects.filter(item => policy.subjectTypes.includes(item.type))) {
        ...
        const activeBlock = registry.blocks.find(item => blockMatches(item, subject, policy.id))
        if (override?.mode === 'block' || activeBlock) {
          const expiresAt = override?.expiresAt || activeBlock?.expiresAt
          return { allowed: false, status: 429, reason: override?.mode === 'block' ? 'block-override' : 'temporarily-blocked', policyId: policy.id, subjectType: subject.type, retryAfterMs: expiresAt ? Math.max(1000, new Date(expiresAt).getTime() - now) : policy.blockMs }
        }
        const key = `${policy.id}:${subjectHash(subject)}`; const cutoff = now - policy.windowMs
        const timestamps = (Array.isArray(registry.counters[key]) ? registry.counters[key] : []).filter(value => Number(value) > cutoff)
        timestamps.push(now); registry.counters[key] = timestamps
        const resetAt = new Date((timestamps[0] || now) + policy.windowMs).toISOString()
        if (timestamps.length > policy.maximum) {
          ...
          registry.blocks.unshift(block); registry.history.unshift({ id: crypto.randomUUID(), action: 'abuse-request.blocked', policyId: policy.id, subjectType: subject.type, route, method, count: timestamps.length, createdAt: nowIso() })
          return { allowed: false, status: 429, reason: 'rate-limit-exceeded', policyId: policy.id, subjectType: subject.type, retryAfterMs: policy.blockMs, limit: policy.maximum, remaining: 0, resetAt }
        }
