...
const persistence = await fs.readFile(path.join(root, 'server/services/authPersistenceService.js'), 'utf8')
...
assert.match(service, /findPersistentSessionById/)
assert.match(service, /await findPersistentSessionById\(connection\.sessionId\)/)
assert.match(service, /Authentication session expired/)
assert.match(service, /webSocketOriginAllowed\(request\)/)
assert.match(service, /KSJ_PORTAL_ORIGIN/)
assert.match(service, /KSJ_ALLOWED_ORIGINS/)
assert.match(service, /WebSocket origin is not approved/)
assert.match(service, /localDevelopmentOrigin/)
...
assert.match(persistence, /export async function findPersistentSessionById\(id\)/)
assert.match(persistence, /status\(session\) === 'active'/)

const originCheckIndex = service.indexOf('if (!webSocketOriginAllowed(request))')
const authenticationIndex = service.indexOf('const session = await findAuthenticationSession(request)')
assert.ok(originCheckIndex >= 0 && authenticationIndex > originCheckIndex, 'WebSocket origin must be verified before cookie-backed authentication')
