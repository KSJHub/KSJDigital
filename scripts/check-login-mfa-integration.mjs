...
try {
  ...
  await storage.writeJson(storage.paths.clients(), [{ ...account, status: 'Suspended' }])
  const blockedCompletion = response()
  await authentication.completeMfaLogin(request({ pendingLoginToken: pendingLogin.body.pendingLoginToken, code: totp(enrollment.secret) }, { 'user-agent': 'Integration Agent' }), blockedCompletion)
  assert.equal(blockedCompletion.statusCode, 401); assert.equal(blockedCompletion.body.error, 'This account is no longer active'); assert.equal(blockedCompletion.headers['Set-Cookie'], undefined)
  await storage.writeJson(storage.paths.clients(), [{ ...account, status: 'Active', canManageMedia: false }])
  const replacementPending = response()
  await authentication.loginWithPassword(request({ email: 'owner@example.test', password: 'correct-password' }, { 'user-agent': 'Integration Agent' }), replacementPending)
  assert.equal(replacementPending.statusCode, 202); assert.equal(replacementPending.body.mfaRequired, true)
  const completed = response()
  await authentication.completeMfaLogin(request({ pendingLoginToken: replacementPending.body.pendingLoginToken, code: totp(enrollment.secret), trustDevice: true, deviceName: 'Integration device' }, { 'user-agent': 'Integration Agent' }), completed)
  assert.equal(completed.statusCode, 200); assert.equal(completed.body.assuranceLevel, 2); assert.ok(completed.body.trustedDeviceToken); assert.match(completed.headers['Set-Cookie'], /ksj_session=/); assert.equal(completed.body.canManageMedia, false)
  const current = response(); await authentication.getCurrentAuthenticationSession(request({}, { cookie: completed.headers['Set-Cookie'].split(';')[0] }), current); assert.equal(current.statusCode, 200); assert.equal(current.body.assuranceLevel, 2); assert.equal(current.body.canManageMedia, false)
  ...
  assert.match(authenticationSource, /function accountIsSuspended/); assert.match(authenticationSource, /accountIsSuspended\(client\)/); assert.match(authenticationSource, /reason: 'account-suspended'/); assert.match(authenticationSource, /currentAccount\(pending\.account\.id\)/); assert.match(authenticationSource, /account-suspended-during-mfa/)
