...
function request(body = {}, headers = {}) {
  return { body, headers, ip: '127.0.0.1', socket: { remoteAddress: '127.0.0.1' } }
}
...
try {
  ...
  await storage.writeJson(storage.paths.clients(), [{ id: 'owner-check', name: 'Owner', email: 'owner@example.test', role: 'owner' }])
  ...
  assert.equal(firstLogin.body.assuranceLevel, 1)
  assert.match(firstLogin.headers['Set-Cookie'], /HttpOnly/)
  ...
  assert.equal(pendingLogin.body.mfaRequired, true)
  assert.ok(pendingLogin.body.pendingLoginToken)

  const oldSession = response()
  authentication.getCurrentAuthenticationSession(request({}, { cookie: firstCookie }), oldSession)
  ...
  assert.equal(completed.body.assuranceLevel, 2)
  assert.ok(completed.body.trustedDeviceToken)
  assert.match(completed.headers['Set-Cookie'], /ksj_session=/)

  const current = response()
  authentication.getCurrentAuthenticationSession(request({}, { cookie: completed.headers['Set-Cookie'].split(';')[0] }), current)
  assert.equal(current.statusCode, 200)
  assert.equal(current.body.assuranceLevel, 2)

  const start = await fs.readFile(path.join(root, 'server/start.js'), 'utf8')
  const router = await fs.readFile(path.join(root, 'server/authenticationRouter.js'), 'utf8')
  assert.match(start, /migratePlaintextCredentials/)
  assert.match(start, /requireAuthenticationSession/)
  assert.match(start, /loginWithPassword/)
  assert.match(start, /requireAssurance\(2\)/)
  assert.match(router, /\/api\/login\/mfa/)
