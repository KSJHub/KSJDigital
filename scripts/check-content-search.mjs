...
try {
  await indexContentRecord(websiteId, 'article', {
    id: 'published', title: 'Diamond launch update', slug: 'diamond-launch', excerpt: 'Production release', category: 'News', tags: ['launch'], author: 'KSJ', locale: 'en-GB', status: 'Published', blocks: [], relatedArticles: [], updatedAt: '2026-07-24T01:00:00.000Z', publishedAt: '2026-07-24T01:00:00.000Z',
  })
  await indexContentRecord(websiteId, 'article', {
  ...
  const publicResults = await searchContent(websiteId, { query: 'diamond' }, { role: 'viewer' })
  ...
  if (editorResults.total !== 2) errors.push('Search visibility hid editable content from an editor')
  ...
  if ('weighted' in (publicResults.results[0] || {})) errors.push('Search results expose internal weighted index data')
