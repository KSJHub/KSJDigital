...
export function updateSiteSection(websiteId, section, values) {
  ...
  return saveSiteConfig(websiteId, { ...config, [section]: { ...config[section], ...values } })
}

export function addNavigationItem(websiteId) {
  const config = getSiteConfig(websiteId)
  const item = { id: `nav-${Date.now()}`, label: 'New Link', target: '/new-link', visible: true, external: false, order: config.navigation.length + 1 }
  return saveSiteConfig(websiteId, { ...config, navigation: [...config.navigation, item] })
}

export function updateNavigationItem(websiteId, id, changes) {
  const config = getSiteConfig(websiteId)
  return saveSiteConfig(websiteId, { ...config, navigation: config.navigation.map(item => item.id === id ? { ...item, ...changes } : item) })
}

export function deleteNavigationItem(websiteId, id) {
  const config = getSiteConfig(websiteId)
  return saveSiteConfig(websiteId, { ...config, navigation: config.navigation.filter(item => item.id !== id).map((item, index) => ({ ...item, order: index + 1 })) })
}
...
export function moveNavigationItem(websiteId, id, direction) {
  ...
  if (index < 0 || nextIndex < 0 || nextIndex >= nav.length) return config
  const [item] = nav.splice(index, 1)
  nav.splice(nextIndex, 0, item)
  return saveSiteConfig(websiteId, { ...config, navigation: nav.map((item, index) => ({ ...item, order: index + 1 })) })
}

export function buildSiteExport(website, pages = [], config = getSiteConfig(website.id)) {
  return {
    website: { id: website.id, name: website.name, domain: website.domain, status: website.status },
