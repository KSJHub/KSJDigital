...
export function PermissionBanner({ account, client = false }) {
  ...
  return <section className="permissionBanner card"><div><span>{client ? 'Client Portal' : 'Owner Portal'}</span><h2>{account.label}</h2><p>{summary.access} · {summary.edit} · {summary.publish}</p></div><div className="permissionPills"><b>{summary.role}</b><small>{account.name}</small></div></section>
}

export function AccessDenied({ account }) {
  return <div className="login"><section className="card accessDenied"><h1>Access Restricted</h1><p>{account.name} does not have permission to open this area.</p><a href={account.home}>Go to dashboard</a></section></div>
}
...
export function ActivityPanel(){return <section className="card activity"><div className="panelHead"><h2>Recent Website Activity</h2><a>View All</a></div>{['Homepage draft saved','New image uploaded','About page edited','Update request created','Support reply received'].map((x,i)=><p key={x}><i></i><b>{x}</b><small>{i+1}h ago</small></p>)}</section>}

export function TicketPanel(){const tickets = getTickets(); return <section className="card tickets"><div className="panelHead"><h2>Support</h2><a>Open Support</a></div>{tickets.slice(0,3).map(([client,subject,priority])=><p key={subject}><b>{subject}<small>{client}</small></b><em>{priority}</em></p>)}</section>}

export function QuickActions({client=false}){const actions=client?[['Edit Website','/client/editor'],['Manage Media','/client/media'],['Request Update','/client/publish'],['Open Support','/client/support'],['Website Settings','/client/settings']]:[['Manage Websites','/owner/websites'],['Add Client','/owner/clients'],['Review Updates','/owner/publish-requests'],['Open Support','/owner/support'],['Settings','/owner/settings']];return <section className="card quick"><h2>Quick Actions</h2><div>{actions.map(([label,path])=><button key={label} onClick={()=>location.href=path}>{label}</button>)}</div></section>}

export function StatusPanel(){return <section className="card status"><h2>Website Health</h2><h3>✓ Ready</h3>{['Website online','Content safe','Media ready','Updates protected'].map(x=><p key={x}><span>✓</span>{x}<small>OK</small></p>)}</section>}

export function PublishPanel(){return <section className="card status"><h2>Updates</h2><h3>Owner approval required</h3>{['Client saves draft','Client requests update','KSJ reviews update','Website goes live'].map(x=><p key={x}><span>✓</span>{x}<small>Step</small></p>)}</section>}
