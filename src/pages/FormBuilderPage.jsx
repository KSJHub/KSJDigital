...
export function FormBuilderPage({ client = false }) {
  ...
  const historicalLiveChanges = previewingHistorical && isPublished && liveConfig && historicalRelease?.snapshot ? draftVsLiveSummary(liveConfig, historicalRelease.snapshot) : []
  ...
  async function publishChanges() {
    ...
    try {
      const next = await api.publishForm(websiteId, formId, { label: cleanReleaseLabel, note: cleanReleaseNote })
      setForms(next)
  ...
  function previewPublishedVersion(release) {
    ...
    setNotice(`Previewing ${release.label || 'published release'} from ${revisionTimestamp(release.publishedAt)}`)
  }

  async function editPublishedVersionDetails(release) {
    if (!canEdit || !websiteId || !selected?.id || !release?.id || busy) return
    const nextLabel = globalThis.prompt('Release label', release.label || 'Published form')
    if (nextLabel === null) return
    const nextNote = globalThis.prompt('Release note (optional)', release.note || '')
    if (nextNote === null) return
    const editedBy = String(account?.displayName || account?.name || account?.id || 'Unknown user').trim().slice(0, 120) || 'Unknown user'
    setBusyAction(`edit-publish-${release.id}`); setNotice('Saving release details')
    try {
      const next = await api.updateFormPublishDetails(websiteId, selected.id, release.id, { label: nextLabel, note: nextNote, editedBy })
      setForms(next)
      setSelectedId(selected.id)
      setNotice('Release details updated')
    } catch (error) {
      setNotice(error.message || 'Release details update failed')
      await loadForms(selected.id, error.message || 'Release details update failed')
    } finally { setBusyAction('') }
  }
        ...
        <section className="card formEditor">
          ...
          {selected && <>
            <div className="formSectionsEditor">
              ...
              {canEdit && !isArchived && <div className="formSettings"><label>Release Label <small>(optional)</small><input value={releaseLabel} maxLength={120} disabled={busy} placeholder={isPublished ? 'e.g. Added phone field' : 'e.g. Contact Form v1'} onChange={event => setReleaseLabel(event.target.value)} /></label><label className="formSettingsWide">Release Note <small>(optional)</small><textarea value={releaseNote} maxLength={500} disabled={busy} placeholder="Short note explaining this release" onChange={event => setReleaseNote(event.target.value)} /></label></div>}
