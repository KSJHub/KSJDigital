...
function DeploymentCard({ deployment, actionLabel, onAction }) {
...
export default function PortalsAdminDeployments() {
  ...
  function cancelDeployment(deployment) {
    ...
    const cancelledDeployment = { ...deployment, status: 'Cancelled', completedAt: 'Just now' };
    const nextData = {
      ...portalData,
      deploymentQueue: deploymentQueue.map((item) => item.id === deployment.id ? cancelledDeployment : item),
      deploymentHistory: [
        { id: `history-${Date.now()}`, websiteId: deployment.websiteId, deploymentId: deployment.id, commitSha: deployment.githubCommitSha ?? '', actor: user?.name ?? 'KSJ Digital Admin', status: 'Cancelled', startedAt: deployment.startedAt ?? 'Not started', completedAt: 'Just now', duration: 'Cancelled before worker execution' },
        ...deploymentHistory,
      ],
      activityLogs: [{ id: `activity-${Date.now()}`, type: 'deployment.cancelled', label: 'Deployment cancelled', actor: user?.name ?? 'KSJ Digital Admin', target: getWebsiteName(deployment.websiteId), timestamp: 'Just now' }, ...(portalData.activityLogs ?? [])],
    };
    setPortalData(savePortalData(nextData));
  }
      ...
      <section className="portal-dashboard-frame" aria-label="Portal deployments">
        ...
        <div className="portal-dashboard-main">
          ...
          <section className="portal-editor-panel">
            <div className="portal-editor-header">
              <div>
                ...
                <h2>Queue, History & Commit Tracking</h2>
                <p>Approved publishes now create deployment jobs, record GitHub commit metadata, and update each website deployment status ready for the real VPS worker phase.</p>
              </div>
            </div>
          ...
          <div className="portal-grid-two">
            <section className="portal-editor-panel">
              <div className="portal-editor-header"><div><p className="eyebrow">Deployment Queue</p><h2>Queued Jobs</h2><p>Jobs created by publish approval before the real VPS worker takes over.</p></div></div>
              <div className="portal-section-list">
                {visibleQueue.length ? visibleQueue.map((deployment) => <DeploymentCard key={deployment.id} deployment={deployment} actionLabel="Cancel" onAction={['Queued', 'Running'].includes(deployment.status) ? cancelDeployment : null} />) : <article><div><div className="portal-section-title-row"><strong>No deployment jobs</strong><span>Waiting</span></div><p>Approve and publish a request to create the first deployment queue item.</p></div></article>}
              </div>
            </section>

            <section className="portal-editor-panel">
              <div className="portal-editor-header"><div><p className="eyebrow">Active Deployments</p><h2>Running / Queued</h2><p>Active jobs are separated so the future worker can surface live progress here.</p></div></div>
              <div className="portal-section-list">
                {activeDeployments.length ? activeDeployments.map((deployment) => <DeploymentCard key={deployment.id} deployment={deployment} />) : <article><div><div className="portal-section-title-row"><strong>No active deployments</strong><span>Idle</span></div><p>There are no queued or running deployment jobs right now.</p></div></article>}
              </div>
            </section>
          </div>
          ...
          <div className="portal-grid-two">
            <section className="portal-editor-panel">
              <div className="portal-editor-header"><div><p className="eyebrow">Deployment History</p><h2>Completed Jobs</h2><p>Successful, failed, and cancelled deployment records with commit and actor metadata.</p></div></div>
              <div className="portal-section-list">
                {visibleHistory.length ? visibleHistory.map((deployment) => <DeploymentCard key={deployment.id} deployment={deployment} />) : <article><div><div className="portal-section-title-row"><strong>No deployment history</strong><span>Waiting</span></div><p>History appears after the first tracked publish or cancellation.</p></div></article>}
              </div>
            </section>

            <section className="portal-editor-panel">
              <div className="portal-editor-header"><div><p className="eyebrow">Failed Deployments</p><h2>Needs Attention</h2><p>Failures are grouped here so the real worker phase can add retry controls later.</p></div></div>
              <div className="portal-section-list">
                {failedDeployments.length ? failedDeployments.map((deployment) => <DeploymentCard key={`${deployment.id}-${deployment.deploymentId ?? ''}`} deployment={deployment} />) : <article><div><div className="portal-section-title-row"><strong>No failed deployments</strong><span>Clear</span></div><p>Nothing needs manual intervention right now.</p></div></article>}
              </div>
            </section>
          </div>
