...
export default function PortalsPublishRequests() {
  ...
  async function publishRequest() {
    ...
    const deploymentStatus = apiResult.ok ? 'Queued' : 'Failed';
    const deploymentMessage = apiResult.ok
      ? `GitHub content commit ${apiResult.commitSha} created. Deployment queued for VPS worker.`
      : `GitHub content write failed/prepared. Deployment cannot run until the write is resolved: ${apiResult.message}`;
    ...
    const nextDeploymentJob = { id: deploymentId, websiteId: selectedRequest.websiteId, publishRequestId: selectedRequest.id, status: deploymentStatus, createdAt: deploymentCreatedAt, startedAt: '', completedAt: '', actor, githubCommitSha: apiResult.ok ? apiResult.commitSha : '', workerMessage: deploymentMessage, source: 'publish-approval' };
    const nextDeploymentHistory = apiResult.ok ? [] : [{ id: `history-${deploymentId}`, websiteId: selectedRequest.websiteId, deploymentId, commitSha: '', actor, status: 'Failed', startedAt: deploymentCreatedAt, completedAt: new Date().toISOString(), duration: 0, workerMessage: deploymentMessage }];
    const nextActivity = createActivity(apiResult.ok ? 'deployment.queued' : 'deployment.failed', apiResult.ok ? `Deployment queued for GitHub commit ${apiResult.commitSha}` : `Deployment blocked for ${contentWrite.contentFilePath}`, actor, selectedRequest.title);
    ...
    commitPortalData({
      ...
      notifications: [{ id: `notice-${Date.now()}`, type: apiResult.ok ? 'deployment' : 'content-write', level: apiResult.ok ? 'success' : 'warning', message: apiResult.ok ? `${selectedWebsite?.name ?? selectedRequest.websiteId} deployment queued for VPS worker after GitHub commit ${apiResult.commitSha}.` : `${selectedWebsite?.name ?? selectedRequest.websiteId} deployment blocked/prepared for ${contentWrite.contentFilePath}. ${apiResult.message}` }, ...(portalData.notifications ?? [])],
    });
    setPublishApiNotice(apiResult.ok ? `Deployment queued. GitHub commit created: ${apiResult.commitSha}` : `Deployment blocked/prepared. ${apiResult.message}`);
    ...
    <main className="portals-shell portals-dashboard-page">
      <section className="portal-dashboard-frame" aria-label="Portal publish requests">
        ...
        <div className="portal-dashboard-main">
          ...
          <section className="portal-editor-panel"><div className="portal-editor-header"><div><p className="eyebrow">Git Content Publishing</p><h2>Safe Publishing Workflow</h2><p>{canApprovePublish ? 'Publishing updates portal live content, creates a 48-hour backup, creates a queued deployment job, then waits for the VPS worker to run the build/deploy step.' : 'Track submitted publish requests. Approval controls are hidden for your role.'}</p></div></div></section>
