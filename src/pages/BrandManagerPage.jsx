    ...
    <Layout client={client} title="Branding">
      <section className="moduleHero card">
        <div>
          ...
          <h2>{selectedName}</h2>
          <p>
            Upload logos, favicons, banners, icons, fonts and documents. These assets will power the
            portal and assigned website brand kit.
          </p>
        </div>
      ...
      <section className="brandManagerGrid">
        <aside className="card brandPicker">
          ...
          {!client && (
            ...
            >
              KSJ Digital System<small>Portal logo, colours and icons</small>
            </button>
          )}
          {websites.map(site => (
            <button
        ...
        <section className="card brandAssets">
          <div className="panelHead">
            <h2>Assets</h2>
            <button>{acceptedBrandFormats.length} formats supported</button>
          </div>
          <div className="brandSlotGrid">
            {brandSlots.map(slot => (
              <article className="brandSlot" key={slot.id}>
                <div className="brandPreview">
                  <AssetPreview asset={assets[slot.id]} />
                </div>
                <div>
                  <b>{slot.label}</b>
                  ...
                  {assets[slot.id] && (
                    <small>
                      {assets[slot.id].name} · {formatSize(assets[slot.id].size)}
                    </small>
                  )}
                </div>
                <label>
        ...
        <aside className="card brandRules">
          ...
          <p>Executable files are not supported for safety.</p>
          {websiteId === 'system' && (
            <div className="systemBrandEditor">
              <h2>System Colours</h2>
              <label>
                Primary
                <input
                  value={systemBrand.primary}
                  onChange={event =>
                    setSystemBrand({ ...systemBrand, primary: event.target.value })
                  }
                />
              </label>
              <label>
                Accent
                <input
                  value={systemBrand.accent}
                  onChange={event => setSystemBrand({ ...systemBrand, accent: event.target.value })}
                />
              </label>
              <button onClick={saveColours}>Save System Brand</button>
            </div>
          )}
        </aside>
      </section>
