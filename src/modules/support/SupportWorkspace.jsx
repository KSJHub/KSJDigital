  ...
  return (
    <Layout client={client} title="Support">
      <section className="supportHero card">
        <div>
          <span>{client ? 'Client Support' : 'Support Desk'}</span>
          <h2>{client ? 'How can KSJ Digital help?' : 'Support Inbox'}</h2>
          <p>
            {client
              ? 'Open a ticket, ask for help, or track website requests from your portal.'
              : 'Manage client conversations, priorities and support history.'}
          </p>
        </div>
      ...
      <div className="supportStats">
        ...
        <article className="card">
          ...
          <strong>1</strong>
          <small>Client response needed</small>
        </article>
        <article className="card">
          <span>Resolved</span>
          <strong>8</strong>
      ...
      <section className="supportDesk">
        <aside className="card ticketInbox">
          <div className="panelHead">
            <h2>Tickets</h2>
            <button>Filter</button>
          </div>
          {tickets.map(ticket => (
            <article key={ticket[0]}>
              <div>
                <b>{ticket[0]}</b>
        ...
        <section className="card conversation supportConversation">
          <div className="panelHead">
            <h2>Homepage banner update</h2>
            <button>High Priority</button>
          </div>
          <div className="message clientMessage">
            <b>{client ? 'You' : 'Client'}</b>
            <p>Can we update the homepage banner before launch?</p>
          </div>
          <div className="message ksjMessage">
            <b>KSJ Digital</b>
            <p>Yes. Upload the replacement image and we will prepare the draft for review.</p>
          </div>
        ...
        <aside className="card helpPanel">
          <h2>Help Centre</h2>
          {helpCards.map(card => (
            <article key={card[0]}>
              <b>{card[0]}</b>
